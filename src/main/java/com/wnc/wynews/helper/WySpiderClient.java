...
public class WySpiderClient extends AbstractHttpClient implements IHttpClient {
    private static Logger logger = Logger.getLogger(WySpiderClient.class);
    ...
    public static WySpiderClient getInstance() {
        if (instance == null) {
            Class var0 = WySpiderClient.class;
    ...
    private void intiThreadPool() {
        this.localDataSaveThreadPool = new SimpleThreadPoolExecutor(10, 10, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue(), "localDataSaveThreadPool");
        this.netPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize, Config.downloadThreadSize, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue(), "netPageThreadPool");
        (new Thread(new ThreadPoolMonitor(this.netPageThreadPool, "netPageThreadPool"))).start();
    ...
    public void manageHttpClient() {
        while (true) {
            System.err.println("抓取总数：" + parseCount.get() + " 闲置代理数:" + ProxyPool.proxyQueue.size());
