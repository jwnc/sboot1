...
public class FunnyCmtTask extends AbstractPageTask {
	...
	protected void retry() {
		...
		SpiderHttpClient.getInstance().getNetPageThreadPool().execute(new FunnyCmtTask(funnyCommetSpy, news));
	}
	...
	protected void handle(Page page) throws Exception {
		...
		int hot = JSONObject.parseObject(page.getHtml()).getIntValue("hot_num");
		if (hot >= HOT_AT_LEAST) {
	...
	private List<HotComment> parseCommentList(String pinglun) throws Exception {
		...
		String jsonHtml = SpiderUtils.getHtmlByProxy(getHotListUrl(pinglun), currentProxy);
		// 针对返回404找不到评论的新闻
		if (("404 Not Found").equals(Jsoup.parse(jsonHtml).title())) {
			return list;
		}

		List<HotComment> parseArray = JSONArray.parseArray(jsonHtml, HotComment.class);
	...
	private String getHotListUrl(String pinglun) {
		return String.format(Zb8Const.COMMENT_FORMAT, pinglun.replace("-", "/"), "hot") + Math.random();
	}

	private static String getHotCountUrl(String pinglun) {
		return String.format(Zb8Const.COMMENT_FORMAT, pinglun.replace("-", "/"), "count") + Math.random();
	}
