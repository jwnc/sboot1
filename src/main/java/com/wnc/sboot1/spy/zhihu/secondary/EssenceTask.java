...
public class EssenceTask {
	...
	private static void seek() throws IOException, InterruptedException {
		...
		while (true) {
			...
			BasicFileUtil.writeFileString("D:\\个人工作\\spy\\zhihu\\topics\\表情图\\表情图-" + skip + ".json", webPage, null,
					false);
			boolean is_end = JSONObject.parseObject(webPage).getJSONObject("paging").getBooleanValue("is_end");
			if (is_end) {
