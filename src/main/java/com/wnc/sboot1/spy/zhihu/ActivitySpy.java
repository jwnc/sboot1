...
public class ActivitySpy {
	...
	public void spy() throws Exception {
		...
		while (true) {
			if (netPageThreadPool.getActiveCount() == 0 || VUSerPageTask.cmtTopicCount >= vCount
					|| getSpyDuration() >= FOUR_HOURS) {
				break;
			}
			Thread.sleep(3000);
		}
		BasicFileUtil.writeFileString("c:/zhihu-task.log", "开始于:" + startTime + " 结束于:" + System.currentTimeMillis(),
				null, true);
	...
	private void initLastSeekTime() {
		try {
			...
			if (readFrom.size() > 0) {
				...
				LAST_SEEK_TIME = BasicNumberUtil.getLongNumber(PatternUtil.getFirstPattern(string, "\\d+"));
