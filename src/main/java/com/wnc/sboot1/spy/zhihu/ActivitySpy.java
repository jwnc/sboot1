...
public class ActivitySpy {
	...
	// 第一次取全量, 下次取增量, 从数据库取
	public static long LAST_SEEK_TIME = 0L;
	...
	public void spy() throws Exception {
		...
		for (String utoken : readFrom) {
			netPageThreadPool.execute(new VUSerPageTask(
					"https://www.zhihu.com/api/v4/members/" + utoken + "/activities", utoken, true, this));
		}

		while (true) {
			Thread.sleep(3000);
			if (VUSerPageTask.cmtTopicCount >= vCount || netPageThreadPool.getActiveCount() == 0
					|| getSpyDuration() > 1000 * 3600 * 4L) {
				break;
			}
		}
	...
	private void initLastTime() {
		try {
			...
			LAST_SEEK_TIME = BasicNumberUtil.getLongNumber(PatternUtil.getFirstPatternGroup(string, "\\d+"));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (LAST_SEEK_TIME < (System.currentTimeMillis() - 1000 * 3600 * 24 * 10L)) {
				LAST_SEEK_TIME = System.currentTimeMillis() - 1000 * 3600 * 24 * 1L;
			}
		}
	}
