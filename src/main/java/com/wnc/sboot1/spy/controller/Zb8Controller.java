...
public class Zb8Controller {
	 ...
	 */
	@GetMapping("comments")
	public String comments(Model model, Integer page, String newsId, String day, Integer orderNews) {
		try {
			...
			if (StringUtils.isNotBlank(day)) {
				pagination = hotCommentService.paginationByDay(page, size, day, orderNews);
			} else if (StringUtils.isNotBlank(newsId)) {
				pagination = hotCommentService.paginationByNid(page, size, newsId);
			}
			if (pagination != null) {
				model.addAttribute("pageData", new PageDataBean<HotComment>(removeDuplicate(pagination),
						pagination.getNumber() + 1, pagination.getSize(), (int) pagination.getTotalElements()));
			}
	...
	private List<HotComment> removeDuplicate(Page<HotComment> pagination) {
		...
		for (HotComment hotComment : content) {
			...
			if (lastNews != null && zb8News.getPinglun().equals(lastNews.getPinglun())) {
