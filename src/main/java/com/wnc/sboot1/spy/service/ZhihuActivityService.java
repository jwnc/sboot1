...
public class ZhihuActivityService
{
    ...
    public static final int FOLLOW_DAY_COUNT = 10;

    public static final int FOLLOW_WEEK_COUNT = 30;

    public static final int FOLLOW_MONTH_COUNT = 50;

    public static final int FOLLOW_YEAR_COUNT = 200;
    ...
    public void aggreLastWeek()
    {
        ...
        aggre(SpiderUtils.wrapDayWithLine(lastMonday), SpiderUtils.wrapDayWithLine(lastSunday),
            AGGRE_WEEK_CODE, FOLLOW_WEEK_COUNT);
    }
    ...
    public void aggreThisWeek()
    {
        ...
        aggre(SpiderUtils.wrapDayWithLine(thisMonday), SpiderUtils.wrapDayWithLine(today),
            AGGRE_WEEK_CODE, FOLLOW_WEEK_COUNT);
    }
    ...
    public void aggreMonth()
    {
        ...
        aggre(SpiderUtils.wrapDayWithLine(year + month + "01"),
            SpiderUtils.wrapDayWithLine(year + month + currentMonthLastDay), AGGRE_MONTH_CODE,
            FOLLOW_MONTH_COUNT);
    }
    ...
    public void aggreLastYear()
    {
        ...
        aggre(SpiderUtils.wrapDayWithLine(lastYear + "0101"),
            SpiderUtils.wrapDayWithLine(lastYear + "1231"), AGGRE_YEAR_CODE, FOLLOW_YEAR_COUNT);
    }
    ...
    public void aggreYear()
    {
        ...
        aggre(SpiderUtils.wrapDayWithLine(year + "0101"),
            SpiderUtils.wrapDayWithLine(year + "1231"), AGGRE_YEAR_CODE, FOLLOW_YEAR_COUNT);
    }
     ...
     * @param count
     */
    public void aggre(final String day1, final String day2, int aggreCode, int count)
    {
        String sql = "select act.cnt,tar.tid tid,tar.info title,tar.type from (SELECT target_id, count(*) cnt FROM zh_activity WHERE FROM_UNIXTIME(created_time) >= '"
                     ...
                     + " 23:59:59' group by target_id having count(*) > " + count
                     + ") act, zh_target tar where act.target_id=tar.tid";
        Query createNativeQuery = entityManager.createNativeQuery(sql);
        List resultList = createNativeQuery.getResultList();
        logger.info(String.format("Day:%s AggreCode:%d  Size:%d SQL:%s", day1, aggreCode,
            resultList.size(), sql));
    ...
    public List getAggreData(String dateStr, int aggreCode)
    {
        String sql = "select ti.cnt,td.description,td.tid,td.title,td.type,td.url from target_aggre_info ti, target_desc td where ti.tid = td.tid and aggre_code="
                     + aggreCode + " and date_str = '" + dateStr + "' order by cnt desc";
        Query createNativeQuery = entityManager.createNativeQuery(sql);
                ...
                case "answer":
                    ...
                    if (findOne2 == null)
                    {
                        HttpGet request = new HttpGet(
                            "https://api.zhihu.com/questions/" + getIdInTid(question_id));
                    ...
                    url = "https://www.zhihu.com/question/" + getIdInTid(question_id) + "/answer/"
                          + idInTid;
                    break;
                case "article":
                    Article article = articleRepository.findOne(tid);
                    description = article.getExcerpt_new();
                    url = "https://zhuanlan.zhihu.com/p/" + idInTid;
                    break;
                case "question":
                    Question question = questionRepository.findOne(tid);
                    description = question.getExcerpt();
                    url = "https://www.zhihu.com/question/" + idInTid;
                    break;
                case "topic":
                    url = "https://www.zhihu.com/topic/" + idInTid + "/hot";
                    break;
                case "column":
                    ZColumn column = columnRepository.findOne(tid);
                    description = column.getIntro();
                    url = "https://zhuanlan.zhihu.com/" + idInTid;
                    break;
                case "collection":
                    url = "https://www.zhihu.com/collection/" + idInTid;
                    break;
                case "roundtable":
    ...
    public String getIdInTid(String tid)
    {
        return PatternUtil.getFirstPatternGroup(tid, "(\\S+)\\+");
    }
}
