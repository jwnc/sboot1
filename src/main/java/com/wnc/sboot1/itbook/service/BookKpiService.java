...
public class BookKpiService
{
    ...
    public List column( String dayStart, String dayEnd )
    {
        ...
        for ( int i = 1; i <= selectAllSqlMap.size(); i++ )
        {
            ...
            rowMap.put( "COUNT", BasicNumberUtil.getNumber( rowMap
                    .get( "COUNT" ).toString() ) );
            list.add( rowMap );
        }
        return list;
    ...
    public List line( String dayStart, String dayEnd )
    {
        ...
        for ( int i = 1; i <= selectAllSqlMap.size(); i++ )
        {
            ...
            if ( !rowMap.get( "DAY" ).toString().equals( lastDay ) )
            {
                ...
                daydataMap.put( "day", BasicDateUtil.getGBWeekString( lastDay
                        .replace( "-", "" ) ) );
                list.add( daydataMap );
            }
            daydataMap.put(
                    rowMap.get( "NAME" ),
                    rowMap.get( "CNT" ) == null ? 0 : BasicNumberUtil
    ...
    public List line_phone( String dayStart, String dayEnd )
    {
        ...
        for ( int i = 1; i <= selectAllSqlMap.size(); i++ )
        {
            rowMap = (Map)selectAllSqlMap.get( i );
            rowMap.put( "CNT",
                    BasicNumberUtil.getNumber( rowMap.get( "CNT" ).toString() ) );
            list.add( rowMap );
        }
        return list;
    }
    ...
    private String dealDayEnd( String dayEnd )
    {
        ...
        {
            dayEnd = BasicDateUtil.getCurrentDateTimeString().substring( 0, 10 );
        }
    ...
    public List lineWeek()
    {
        ...
        String weekBeforeToday = BasicDateUtil.getDateBeforeDayDateString(
                today, 6 );
