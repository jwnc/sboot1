...
public class MailSendTask
{
    ...
    public void sendTemplateMail()
    {
        ...
        send( "七日阅读统计", BasicDateUtil.getDateBeforeDayDateString( today, 6 )
                + "-" + today + ")", bookKpiService.lineWeek4Mail() );
    }
    ...
    {
        send( "昨日阅读统计",
                BasicDateUtil.getDateBeforeDayDateString( getToday(), 1 ),
    ...
    private void send( String summary, String dateStr, List list )
    {
        ...
        try
        {
            ...
            {
                i++;
                boolean sendHtmlMail = mailService.sendHtmlMail( emailTo,
                        summary + "(" + dateStr + ")", emailContent );
                if ( sendHtmlMail )
                {
                    break;
                }
                LOGGER.info(
                        "第" + i + "次发送失败, 过" + retryIntervalMinutes + "分钟重试" );
