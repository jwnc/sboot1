...
public class ProxyUtil {
	...
	public static void main(String[] args) throws IOException {
		if (ProxyUtil.checkNetWork()) {
			boolean checkAvailable = ProxyUtil.checkAvailable("159.65.184.229:3128");
	...
	public static boolean checkAvailable(String proxyStr) throws IOException {
		...
		if (StringUtils.isNotBlank(proxyStr)) {
			...
			final Proxy currentProxy = new Proxy(split[0], Integer.parseInt(split[1]), 1000L);
	...
	public static List<String> get61Proxies() throws IOException {
		...
		List<String> list = PatternUtil.getAllPatternGroup(content, "\\d+.\\d+.\\d+.\\d+:\\d+");
