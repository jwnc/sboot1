...
public class ProxyHashService {
	...
	// 构造函数使用@Autowired
	@Autowired
	public ProxyHashService(StringRedisTemplate stringRedisTemplate, RedisTemplate redisTemplate) {
		checkCount = 0;
	...
	public List<String> getFatestProxies(int count) {
		...
		Collections.sort(map2List, new Comparator<KeyValue>() {
			...
			public int compare(KeyValue o1, KeyValue o2) {
				if (o1.getV() instanceof Integer && o2.getV() instanceof Integer) {
					return Integer.parseInt(o1.getV() + "") - Integer.parseInt(o2.getV() + "");
				}
				return 0;
			}
		});
		List<String> ret = new ArrayList<String>();
		List<KeyValue> subList = map2List.subList(0, Math.min(count, map2List.size()));
	...
	public void checkAll() {
		...
		for (final Map.Entry<String, Integer> entry : availableProxies.entrySet()) {
			threadPoolExecutor.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						checkCount++;
						if (!checkAvailable) {
							logger.info("已check数目:" + checkCount + ", 代理失效:" + proxyStr);
							opsForHash.put(redisHashKey, proxyStr, INVALID_PROXY_REPLYTIME);
						} else {
							System.out.println("已check数目:" + checkCount + ", 代理有效:" + proxyStr);
							opsForHash.put(redisHashKey, proxyStr, (int) (System.currentTimeMillis() - t1));
						}
					}
				}
			});
