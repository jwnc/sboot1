...
public class DictionaryDao
{
    ...
    public synchronized static DicWord findWord( String word )
    {
        ...
        try
        {
            String sql = "select e.word_done,e.word_er,e.word_est,e.word_ing,e.word_pl,e.word_past,e.word_third,d.id,d.topic_id,d.topic_word,d.mean_cn,d.weight FROM  dictionary D LEFT JOIN word_exchange E  ON E.dict_id=D.id where LOWER(topic_word)='"
                    + word + "' or  word_third='" + word + "' or  word_done='"
                    + word + "' or  word_er='" + word + "' or  word_est='"
                    + word + "' or  word_ing='" + word + "' or  word_pl='"
    ...
    public static DicWord findWeightWord( String word )
    {
        ...
        {
            if ( hasFind( word, dw.getBase_word() )
                    || hasFind( word, dw.getWord_er() )
                    || hasFind( word, dw.getWord_est() )
                    || hasFind( word, dw.getWord_done() )
                    || hasFind( word, dw.getWord_ing() )
                    || hasFind( word, dw.getWord_past() )
                    || hasFind( word, dw.getWord_pl() )
    ...
    public static boolean insertExchange( String word_org,
            WordExchange wordExchange ) throws SQLException
    {
        ...
        util.addInsertField(
                new DbField( "WORD_THIRD", wordExchange.getWord_third() ) );
        util.addInsertField(
                new DbField( "WORD_DONE", wordExchange.getWord_done() ) );
        util.addInsertField(
                new DbField( "WORD_PL", wordExchange.getWord_pl() ) );
        util.addInsertField(
                new DbField( "WORD_ING", wordExchange.getWord_ing() ) );
        util.addInsertField(
                new DbField( "WORD_PAST", wordExchange.getWord_past() ) );
        util.addInsertField(
                new DbField( "WORD_EST", wordExchange.getWord_est() ) );
        util.addInsertField(
                new DbField( "WORD_ER", wordExchange.getWord_er() ) );
        BasicFileUtil.writeFileString( "exchange_sql.txt",
                util.getInsertSql() + "\r\n", null, true );
