...
public class DbFieldSqlUtil
{
    ...
    public boolean addSelectField( DbField paramDbField )
    {
        ...
        if ( (paramDbField != null)
                && !(BasicStringUtil.isNull2String( paramDbField.fieldCode,
                        paramDbField.fieldType )) )
        {
            if ( !this.map3.containsKey( paramDbField.fieldCode ) )
            {
                ...
                {
                    selectField += paramDbField.fieldCode + " AS "
                            + paramDbField.fieldValue.replace( "'", "" ) + ",";
                }
    ...
    public String getInsertSql()
    {
        ...
        for ( int i = 0; i < this.vcInsertField.size(); ++i )
        {
            ...
            if ( !BasicStringUtil.isNull2String( localDbField.fieldCode,
                    localDbField.fieldType ) )
            {
                if ( localDbField.fieldValue != null )
                {
                    if ( "STRING".equalsIgnoreCase( localDbField.fieldType ) )
                    {
                        ...
                        str3 = str3 + "'" + localDbField.fieldValue + "',";
                            ...
                            .equalsIgnoreCase( localDbField.fieldType ) )
                    {
                        str2 = str2 + localDbField.fieldCode + ",";
                        str3 = str3 + localDbField.fieldValue + ",";
                    } else if ( "NUMBER"
        ...
        {
            str1 = "INSERT INTO " + this.tableCode + "("
                    + str2.substring( 0, str2.length() - 1 ) + ")VALUES(";
    ...
    public String getSelectSql()
    {
        ...
        try
        {
            if ( !BasicStringUtil.isNullString( this.tableCode ) )
            {
                ...
                {
                    str1 = "SELECT " + this.selectField + " FROM "
                            + this.tableCode;
                } else
                {
    ...
    public String getUpdateSql()
    {
        ...
        try
        {
            if ( !BasicStringUtil.isNullString( this.tableCode ) )
            {
                ...
                {
                    if ( !BasicStringUtil.isNullString(
                            str3 = (localDbField = this.vcUpdateField.get( i ))
                                    .getSelectFieldString() ) )
                    {
                        if ( !BasicStringUtil
                                .isNullString( (String)localObject ) )
                        {
                ...
                if ( !BasicStringUtil.isNullString( (String)localObject ) )
                {
                    ...
                    str1 = "UPDATE " + this.tableCode + " SET "
                            + (String)localObject;
    ...
    public String getWhereSql()
    {
        ...
        {
            if ( !BasicStringUtil.isNullString(
                    str = (localDbField = this.vcWhereField.get( i ))
                            .getSelectFieldString() ) )
            {
                if ( !BasicStringUtil.isNullString( (String)localObject ) )
                {
        ...
        if ( !BasicStringUtil.isNullString( this.addWhereFieldSql ) )
        {
            if ( !BasicStringUtil.isNullString( (String)localObject ) )
            {
                localObject = this.addWhereFieldSql + " AND "
                        + (String)localObject;
            } else
            {
